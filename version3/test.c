...
char plateauGlobal[HAUTEUR_PLATEAU][LARGEUR_PLATEAU];
...
int main()
{
    ...
    system("clear");
    disableEcho();
    initPlateau(plateauGlobal);
    placerPaves(plateauGlobal);
    dessinerPlateau(plateauGlobal);
...
void progresser(int lesX[], int lesY[], char direction, bool *collision) {
    *collision = false;
    ...
    if (plateauGlobal[lesY[0]][lesX[0]] == COTE_BORDURE) {
        *collision = true;
        system("clear");
        return;
    }
