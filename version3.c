 ...
 * @details Implémentation d'un jeu de type Snake en C avec des bordures et des pavers
 * @version 2.0
 * @date 17/11/24
 ...
 * et boucle jusqu'à ce que l'utilisateur décide d'arrêter le jeu en appuyant sur STOP_JEU ou s'il mange une bordure ou un obstacle.
...
int main()
{
    ...
    while (condition_arret == TRUE){
        ...
        usleep(VITESSE_JEU);// Pause pour contrôler la vitesse du jeu
    }
...
void progresser(int lesX[], int lesY[], char direction, bool *collision) {
    *collision = false;
    ...
    if (lesX[0] < 0 || lesX[0] >= LARGEUR_PLATEAU || lesY[0] < 0 || lesY[0] >= HAUTEUR_PLATEAU) 
    {
        *collision = true;
        system("clear");
        return;
    ...
    // Gestion des collisions avec les obstacles en utilisant le plateau global
